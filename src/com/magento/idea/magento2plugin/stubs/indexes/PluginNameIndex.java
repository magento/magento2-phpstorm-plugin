...
public class PluginNameIndex extends ScalarIndexExtension<String> {
    public static final ID<String, Void> KEY = ID.create("com.magento.idea.magento2plugin.stubs.indexes.plugin_name");
    ...
    public DataIndexer<String, Void, FileContent> getIndexer() {
        return inputData -> {
            ...
            if (psiFile instanceof PhpFile) {
                grabPluginNamesFromPhpFile((PhpFile) psiFile, map);
            } else if (psiFile instanceof XmlFile) {
                grabPluginNamesFromXmlFile((XmlFile) psiFile, map);
            }
    ...
    private void grabPluginNamesFromPhpFile(PhpFile file, Map<String, Void> map) {
    ...
    private void recursiveFill(List<String> results, PsiElement psiElement) {
        if (PhpPatternsHelper.STRING_METHOD_ARGUMENT.accepts(psiElement) && isInContextOfDispatchMethod(psiElement)) {
            String pluginName= StringUtil.unquoteString(psiElement.getText());
    ...
    private void grabPluginNamesFromXmlFile(XmlFile file, Map<String, Void> map) {
