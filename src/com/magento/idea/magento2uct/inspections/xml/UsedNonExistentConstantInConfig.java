...
public class UsedNonExistentConstantInConfig extends ModuleConfigFileInspection {
    ...
    protected void doInspection(
        ...
        if (!VersionStateManager.getInstance(manager.getProject()).isExists(fqn)) {
            final String message = SupportedIssue.USED_NON_EXISTENT_CONSTANT_IN_CONFIG.getMessage(
                    fqn,
            ...
            if (holder instanceof UctProblemsHolder) {
                ((UctProblemsHolder) holder).setIssue(
                        SupportedIssue.USED_NON_EXISTENT_CONSTANT_IN_CONFIG
                );
            }
